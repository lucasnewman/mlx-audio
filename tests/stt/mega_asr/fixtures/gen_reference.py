...
def select_clean_clip(router: AudioQualityRouter) -> tuple[Path, dict]:
    ...
    for idx, sample in enumerate(ds):
        ...
        print('clean candidate', idx, f'dur={duration:.2f}s', f"prob={info['degraded_prob']:.4f}", flush=True)
    ...
    raise RuntimeError(f'No clean FLEURS sample routed below 0.5; best was idx={best[idx]} prob={best[degraded_prob]:.4f}')
...
def add_noise(audio: np.ndarray, snr_db: float, rng: np.random.Generator) -> np.ndarray:
    ...
    noise = fftconvolve(noise, np.array([1.0, -0.85], dtype=np.float32), mode='same').astype(np.float32)
...
def make_degraded_clip(router: AudioQualityRouter, clean_path: Path) -> tuple[Path, dict]:
    ...
    for attempt, cfg in enumerate(configs):
        ...
        print('degraded attempt', attempt, cfg, f"prob={info['degraded_prob']:.4f}", flush=True)
        if info['degraded_prob'] >= 0.5:
...
def run() -> None:
    ...
    (FIXTURE_DIR / 'reference.json').write_text(json.dumps(reference, indent=2, ensure_ascii=False) + '\n')
    (FIXTURE_DIR / 'router_state_dict_keys.json').write_text(json.dumps(router_state, indent=2, ensure_ascii=False, sort_keys=True) + '\n')
