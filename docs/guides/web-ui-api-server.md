...
Compatible non-streaming TTS `/v1/audio/speech` requests are routed through the server's continuous batching path when the model exposes a `create_tts_batch_session(...)` hook. Qwen3 TTS Base and CustomVoice models currently provide this hook, so later compatible requests can join an active per-model session while earlier requests are still generating.

Requests that are not supported by the model's continuous batching hook continue through the existing serial or fixed-window batch paths. For Qwen3 TTS, this currently excludes streaming requests, reference-audio requests, VoiceDesign requests, and unsupported speed or pitch settings.
