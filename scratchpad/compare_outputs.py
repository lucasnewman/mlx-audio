...
def compute_stft(audio: np.ndarray, sr: int = 48000, n_fft: int = 960, hop: int = 480):
    ...
    f, t, spec = signal.stft(audio, fs=sr, window=window, nperseg=n_fft, noverlap=n_fft-hop)
    return f, t, spec
...
def compute_metrics(orig: np.ndarray, pytorch: np.ndarray, mlx: np.ndarray, sr: int = 48000):
...
def plot_waveform_comparison(orig: np.ndarray, pytorch: np.ndarray, mlx: np.ndarray, sr: int, output_path: str):
...
def plot_spectrogram_comparison(orig: np.ndarray, pytorch: np.ndarray, mlx: np.ndarray, sr: int, output_path: str):
    ...
    im0 = axes[0,0].pcolormesh(t, f, spec_db, shading='gouraud', vmin=vmin, vmax=vmax, cmap='viridis')
    ...
    im1 = axes[0,1].pcolormesh(t, f, spec_db_pt, shading='gouraud', vmin=vmin, vmax=vmax, cmap='viridis')
    ...
    im2 = axes[0,2].pcolormesh(t, f, spec_db_mlx, shading='gouraud', vmin=vmin, vmax=vmax, cmap='viridis')
    ...
    im3 = axes[1,0].pcolormesh(t[:min_len], f, spec_diff, shading='gouraud', vmin=-20, vmax=20, cmap='RdBu_r')
    ...
    im4 = axes[1,1].pcolormesh(t[:min_len], f, reduction_pt, shading='gouraud', vmin=-40, vmax=40, cmap='coolwarm')
    ...
    im5 = axes[1,2].pcolormesh(t[:min_len], f, reduction_mlx, shading='gouraud', vmin=-40, vmax=40, cmap='coolwarm')
...
def plot_rms_comparison(orig: np.ndarray, pytorch: np.ndarray, mlx: np.ndarray, sr: int, output_path: str):
...
def plot_correlation_scatter(pytorch: np.ndarray, mlx: np.ndarray, output_path: str):
    ...
    axes[1].axvline(x=np.mean(diff), color='g', linestyle='-', label=f'Mean: {np.mean(diff):.6f}')
...
def plot_noise_reduction_overlay(orig: np.ndarray, pytorch: np.ndarray, mlx: np.ndarray, sr: int, output_path: str):
    ...
    vmin, vmax = -40, 40
    axes[0].pcolormesh(
        t_plot, f, reduction_pt, shading='gouraud', vmin=vmin, vmax=vmax, cmap='Reds', alpha=0.45
    )
    axes[0].pcolormesh(
        t_plot, f, reduction_mlx, shading='gouraud', vmin=vmin, vmax=vmax, cmap='Blues', alpha=0.45
    )
...
def plot_noise_reduction_absdiff(orig: np.ndarray, pytorch: np.ndarray, mlx: np.ndarray, sr: int, output_path: str):
    ...
    im = ax.pcolormesh(t_plot, f, abs_diff, shading='gouraud', cmap='gray', vmin=0.0, vmax=vmax)
...
def main():
    parser = argparse.ArgumentParser(description="Compare PyTorch and MLX DeepFilterNet outputs")
    ...
    parser.add_argument("-o", "--output-dir", default="comparison_output", help="Output directory for results")
    ...
    if metrics['ser_db'] > 30:
        print(f"Signal-to-Error Ratio ({metrics['ser_db']:.1f} dB) indicates high fidelity.")
    elif metrics['ser_db'] > 20:
        print(f"Signal-to-Error Ratio ({metrics['ser_db']:.1f} dB) indicates good fidelity.")
    else:
        print(f"Signal-to-Error Ratio ({metrics['ser_db']:.1f} dB) indicates some differences.")
    ...
    plot_waveform_comparison(orig, pytorch, mlx, 48000, output_dir / "waveform_comparison.png")
    plot_spectrogram_comparison(orig, pytorch, mlx, 48000, output_dir / "spectrogram_comparison.png")
    ...
    plot_noise_reduction_overlay(orig, pytorch, mlx, 48000, output_dir / "noise_reduction_overlay.png")
    plot_noise_reduction_absdiff(orig, pytorch, mlx, 48000, output_dir / "noise_reduction_absdiff.png")
