...
@dataclass
class _Qwen3PendingContinuousRequest:
    sequence_idx: int
    text: str
...
@dataclass
class _Qwen3ActiveContinuousRequest:
    sequence_idx: int
    text: str
    voice: Optional[str]
...
class _Qwen3ContinuousBatchEngine:
    ...
    def __init__(
        ...
        *,
        ...
        self._pending: List[_Qwen3PendingContinuousRequest] = []
        self._active: List[_Qwen3ActiveContinuousRequest] = []
    ...
    def submit(
        ...
        *,
        ...
        self._pending.append(
            _Qwen3PendingContinuousRequest(
    ...
    def _take_pending_batch(self) -> List[_Qwen3PendingContinuousRequest]:
    ...
    def _admit_pending(self) -> List[BatchGenerationResult]:
        ...
        for index, request in enumerate(pending):
            ...
            state = _Qwen3ActiveContinuousRequest(
    ...
    def _advance_active(self) -> List[BatchGenerationResult]:
        ...
        completed: List[BatchGenerationResult] = []
        still_active: List[_Qwen3ActiveContinuousRequest] = []
    ...
    def _next_input_embeds_for_states(
        self,
        states: List[_Qwen3ActiveContinuousRequest],
        code_tokens: List[mx.array],
    ) -> mx.array:
        text_embeds = []
        for state in states:
    ...
    def _attention_mask_for_states(
        self, states: List[_Qwen3ActiveContinuousRequest]
    ) -> mx.array:
        cache_lengths = [
            state.cache[0].offset if state.cache and state.cache[0].keys is not None else 0
            for state in states
        ]
    ...
    def _decode_state(
        self, state: _Qwen3ActiveContinuousRequest
    ...
    def _empty_result(self, sequence_idx: int) -> BatchGenerationResult:
        ...
        return BatchGenerationResult(
            ...
            sample_rate=self.model.sample_rate,
            token_count=0,
            audio_duration=format_duration(0.0),
            processing_time_seconds=time.time() - self._start_time,
            peak_memory_usage=mx.get_peak_memory() / 1e9,
        )
...
class Model(nn.Module):
    ...
    def create_continuous_batch_engine(
        ...
        *,
        ...
        max_batch_size: int = 8,
        verbose: bool = False,
    ) -> _Qwen3ContinuousBatchEngine:
        return _Qwen3ContinuousBatchEngine(
            self,
