...
class RelativePositionBias(nn.Module):
    ...
    def _relative_position_bucket(relative_position, causal=True, num_buckets=32, max_distance=128):
        ...
        # Calculate val_if_large
        val_if_large = max_exact + (
            mx.log(n.astype(mx.float32) / max_exact) / math.log(max_distance / max_exact) * (num_buckets - max_exact)
        ).astype(mx.int32)
        val_if_large = mx.minimum(val_if_large, mx.full_like(val_if_large, num_buckets - 1))
    ...
    def __call__(self, qk_dots):
        i, j = qk_dots.shape[-2:]
        q_pos = mx.arange(i, dtype=mx.int32)
        k_pos = mx.arange(j, dtype=mx.int32)
...
class AttentionQKV(nn.Module):
    def __init__(self, n_heads, head_dim, dropout_rate=0.1, scale=None, flash=False):
        ...
        self.flash = flash  # MLX doesn't have flash attention in the same way
    ...
    def split_heads(self, x):
        ...
        return mx.moveaxis(x, 1, 2)  # (bs, n_heads, length, head_dim)

    def combine_heads(self, x):
        bs, _, length, _ = x.shape
        x = mx.moveaxis(x, 1, 2)  # (bs, length, n_heads, head_dim)
...
class Perceiver(nn.Module):
    ...
    def __init__(self, pre_attention_query_token=32, pre_attention_query_size=1024, 
        ...
        # In MLX, we need to store this as an attribute that will be recognized as a parameter
    ...
    def __call__(self, h):
        ...
        # MLX uses repeat instead of expand
        query_ = mx.repeat(self.pre_attention_query, h.shape[0], axis=0)
