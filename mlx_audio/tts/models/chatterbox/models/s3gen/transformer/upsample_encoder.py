...
def interpolate_nearest_1d(x: mx.array, scale_factor: float) -> mx.array:
    """Nearest neighbor interpolation for 1D signals.
    ...
    indices = mx.floor(mx.arange(new_length) / scale_factor).astype(mx.int32)
...
class Upsample1D(nn.Module):
    ...
    def __call__(self, inputs: mx.array, input_lengths: mx.array):
        outputs = interpolate_nearest_1d(inputs, scale_factor=float(self.stride))
...
class PreLookaheadLayer(nn.Module):
    ...
    def __call__(self, inputs: mx.array) -> mx.array:
        ...
        """
        outputs = mx.transpose(inputs, (0, 2, 1))
        # look ahead
...
class UpsampleConformerEncoder(nn.Module):
    ...
    def __call__(
        ...
        Returns:
            ...
            masks: mx.array batch padding mask after subsample
        ...
        """
        T = xs.shape[1]
        masks = ~make_pad_mask(xs_lens, T).expand_dims(1)  # (B, 1, T)
    ...
    def forward_layers(self, xs: mx.array, chunk_masks: mx.array, pos_emb: mx.array, mask_pad: mx.array) -> mx.array:
        for layer in self.encoders:
            xs, chunk_masks, _, _ = layer(xs, chunk_masks, pos_emb, mask_pad)
        return xs

    def forward_up_layers(self, xs: mx.array, chunk_masks: mx.array, pos_emb: mx.array, mask_pad: mx.array) -> mx.array:
