...
class EnTokenizer:
    ...
    def text_to_tokens(self, text: str):
        ...
        text_tokens = mx.array(text_tokens.tolist())
        return text_tokens
    ...
    def decode(self, seq):
        txt: str = self.tokenizer.decode(seq.tolist(), skip_special_tokens=False)
        txt = txt.replace(" ", "")
        txt = txt.replace(SPACE, " ")
        txt = txt.replace(EOT, "")
        txt = txt.replace(UNK, "")
...
class MTLTokenizer:
    ...
    def encode(
        ...
        txt = txt.replace(" ", SPACE)
        return mx.array([self.tokenizer.encode(txt).ids], dtype=mx.int32)

    def decode(self, seq):
        txt = self.tokenizer.decode(seq.tolist(), skip_special_tokens=False)
