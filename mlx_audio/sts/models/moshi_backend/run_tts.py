...
def main():
    ...
    parser.add_argument(
        "--nq", type=int, default=32, help="Number of codebooks to generate."
    )
    parser.add_argument(
        "--temp", type=float, default=0.6, help="Temperature for text and audio."
    )
    ...
    parser.add_argument(
        "--max-padding", type=int, default=8, help="Max padding in a row, in steps."
    )
    parser.add_argument(
        "--initial-padding", type=int, default=2, help="Initial padding, in steps."
    )
    parser.add_argument(
        "--final-padding",
        type=int,
    ...
    def _flush():
        ...
        for request in batch:
            entries = tts_model.prepare_script(
                request.turns, padding_between=args.padding_between
            )
            ...
            if prefixes is not None:
                assert (
                    len(request.voices) == 1
                ), "For this model, only exactly one voice is supported."
                prefix_path = hf_get(
                    request.voices[0], args.voice_repo, check_local_file_exists=True
                )
        ...
        log(
            "info",
            f"[LM] Batch of size {len(batch)} took {time_taken:.2f}s, "
        ...
        for idx, request in enumerate(batch):
            ...
            if not args.only_wav:
                mx.save_safetensors(
                    str(filename.with_suffix(".safetensors")), debug_tensors
                )
                debug_info = {
